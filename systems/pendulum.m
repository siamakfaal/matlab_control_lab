classdef  pendulum < handle & basemodel
    properties(Access=public)
        m double = 1; % Mass of the bob [kg]
        l double = 1; % Lenght of the pendulum [m]
        b double = 1; % Damping coefficient [Nms]
    end

    properties(Access=private)
        bob_radius;    
        bob_radius_factor = 0.07; % bob_radius = bob_radius_factor * l
        bob_perimeter;
        bob_perimeter_div = 20;
        link_thinkness_factor = 3; % body_thickness = link_thickness_factors * l
        link_length;
    end

    methods
        function obj = pendulum(varargin)
            obj.name = "Pendulum";  % Default value for name
            obj.x0 = [0;0]; % Default value for x0
            obj.parse_model_inputs(varargin{:});
            obj.set_visual_parameters();
        end

        function dx = openloop(obj,t,x,u), dx = [x(2); obj.M(t,x)\(obj.B(t,x)*u - obj.h(t,x))]; end
        function dx = closedloop(obj,~,x), dx = obj.openloop([],x,0); end
        function E = energy(obj,~,x), E = obj.m*obj.l*(obj.l*x(2)^2/2 + obj.g*cos(x(1))); end
        function mass_matrix = M(obj,~,~), mass_matrix  = obj.m*obj.l^2; end
        function ncsvr = h(obj,~,x), ncsvr = -obj.g*obj.m*obj.l*sin(x(1)) + obj.b*x(2); end
        function inputmatrix = B(~,~,~), inputmatrix = 1; end

        function pose = stick_diagram(obj,x)
            cos_q = cos(x(1));
            sin_q = sin(x(1));
            % Pose 1 = Link
            pose(1).x = [0 -obj.link_length*sin_q];
            pose(1).y = [0 obj.link_length*cos_q];
            pose(1).z = [0 0];
            % Pose 2 = Bob
            pose(2).x = obj.bob_perimeter(:,1) - obj.l*sin_q;
            pose(2).y = obj.bob_perimeter(:,2) + obj.l*cos_q;
            pose(2).z = obj.bob_perimeter(:,3);
        end
    end

        

    methods(Access=private)
        function set_visual_parameters(obj)
            obj.bob_radius = obj.l * obj.bob_radius_factor;
            obj.body_thickness = obj.l * obj.link_thinkness_factor;            
            obj.link_length = obj.l - obj.bob_radius;
            Q = linspace(0, 2*pi, obj.bob_perimeter_div)';
            obj.bob_perimeter = obj.bob_radius*[cos(Q), sin(Q), zeros(size(Q))];
            obj.animation_axes_limits = obj.l*1.2*[-1 1; -1 1;-1 1];
        end
    end

end