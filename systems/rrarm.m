classdef  rrarm < handle & basemodel
    ...
    methods(Access=public)
        function obj = rrarm(varargin)
            ...
            obj.shape_update_function = @(x) obj.robot_kinematics(x);
            
            obj.set_visual_parameters(min(obj.l) * obj.joint_radius_factor, ...
                min(obj.l) * obj.link_thinkness_factor, ...
                1.2*sum(obj.l)*[-1, 1; -1, 1; -1, 1]);
        end
    end
    ...
    methods(Access=public)
        function set_visual_parameters(obj, joint_radius, link_thickness, axes_limits)
            obj.joint_radius = joint_radius;
    ...
    methods(Access=private)
        ...
        function pose = robot_kinematics(obj,x)
            R2 = obj.rot2(x(1)+x(2));
            p0 = [0;0];
